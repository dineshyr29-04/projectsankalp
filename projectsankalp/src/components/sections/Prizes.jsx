...
const Confetti = ({ active }) => {
  ...
  useEffect(() => {
    if (active) {
      const colors = ['#FBBF24', '#FCD34D', '#3B82F6', '#10B981', '#F43F5E', '#8B5CF6'];
    ...
    >
      ...
      >
        ...
        <div className="space-y-1 md:space-y-2 mb-4 md:mb-12 mt-2 md:mt-4">
          <span className={`inline-block px-2 md:px-4 py-0.5 md:py-1.5 rounded-full border ${t.border} bg-white/80 text-[7px] md:text-[10px] font-black uppercase tracking-[0.1em] md:tracking-[0.2em] text-slate-700 shadow-sm`}>
            {prize.rank}
          </span>
          <h3 className={`text-sm sm:text-lg md:text-4xl font-serif font-black ${t.accent} pt-1 md:pt-6`}>
            {prize.material}
          </h3>
...
export default function Prizes() {
  ...
  return (
    <Section id="prizes" className="relative bg-slate-50 py-16 md:py-40 overflow-hidden">
      <style>{`
        @keyframes shimmer {
      ...
      <Container className="relative z-10 px-2 sm:px-10 lg:px-20 mx-auto flex flex-col items-center">
        <header className="text-center mb-12 md:mb-32 w-full">
          ...
          >
            ...
            <span className="text-[9px] md:text-[12px] font-black uppercase tracking-[0.4em] text-blue-600">Rewards</span>
