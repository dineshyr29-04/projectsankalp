...
const PrizeCard = ({ prize, index, isCenter }) => {
  ...
  return (
    <motion.div
      ref={cardRef}
      initial={{ opacity: 0, x: getInitialX(), y: 50, scale: 0.8, rotateZ: index === 0 ? -10 : index === 2 ? 10 : 0 }}
      whileInView={{ opacity: 1, x: 0, y: isCenter ? 0 : 40, scale: isCenter ? 1.1 : 0.9, rotateZ: isCenter ? 0 : index === 0 ? 3 : -3 }}
      viewport={{ once: false, amount: 0.2 }}
      transition={{ type: "spring", bounce: 0.4, duration: 1.5, delay: index * 0.1 }}
      onMouseMove={handleMouseMove}
      onMouseLeave={handleMouseLeave}
      style={{ rotateX, rotateY, zIndex: isCenter ? 10 : 5 }}
      className={`relative w-full max-w-[280px] md:max-w-[320px] aspect-[3/4] cursor-pointer group perspective-1000 mx-auto md:mx-0`}
    >
      {/* Dynamic Ground Shadow */}
      <motion.div 
        className={`absolute -bottom-12 left-1/2 -translate-x-1/2 w-4/5 h-10 blur-2xl rounded-full transition-opacity duration-500 ${theme.glow} opacity-0 group-hover:opacity-100`}
      />

      {/* Glass Body */}
      <div className={`relative w-full h-full rounded-3xl overflow-hidden shadow-2xl transition-all duration-300 group-hover:shadow-3xl group-hover:${theme.glow} border ${theme.border} bg-white/10 backdrop-blur-xl isolate`}>
        ...
        <motion.div 
          className="absolute inset-0 bg-gradient-to-tr from-white/0 via-white/30 to-white/0 opacity-0 group-hover:opacity-100 pointer-events-none rounded-3xl"
        ...
        <div className="absolute inset-0 p-8 flex flex-col justify-between">
          <div className="space-y-2">
            <span className="inline-block px-4 py-1.5 rounded-full border border-white/40 bg-white/30 backdrop-blur-md text-[10px] font-black uppercase tracking-[0.3em] text-slate-800 shadow-sm">
              {prize.rank}
            </span>
            <h3 className={`text-2xl md:text-3xl font-serif font-black ${theme.text} italic pt-2`}>
              {prize.material}
            </h3>
          </div>
          ...
          <div className="text-center relative">
            ...
            <motion.div 
              className="text-4xl md:text-5xl lg:text-6xl font-serif font-black text-slate-900 tracking-tighter"
        ...
        <div className="absolute inset-0 rounded-3xl border-2 border-white/20 pointer-events-none" style={{ maskImage: 'linear-gradient(to bottom right, black, transparent)' }} />
      </div>
    </motion.div>
  );
};
