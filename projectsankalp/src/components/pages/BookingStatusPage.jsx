...
export default function BookingStatusPage({ slots, occupancy: propOccupancy, onBack, onDelete, onCheckIn }) {
        ...
        try {
          await html5QrCode.start(
            ...
            { 
              ...
              aspectRatio: 1.0,
              disableFlip: true // Speed boost
            },
            (result) => {
              const now = Date.now();
              // Debounce to prevent multiple rapid writes for same ID
  ...
  const handleScanSuccess = (result) => {
    ...
    if (matchingSlot && matchingSlot.docId && !matchingSlot.checkedIn) {
      ...
      // Visual feedback: brief pulse on the scanner modal or vibrate
            ...
            <div key={domain.id} className="flex flex-col min-w-0">
              <div className="flex items-center justify-between px-4 mb-6">
                <div className="min-w-0">
                  ...
                  <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest mt-1 block">Sector Registry Alpha</span>
                </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4 flex-1 min-w-0">
                        <span className={`text-[10px] font-mono font-black shrink-0 ${slot.checkedIn ? "text-emerald-500" : isHighlighted ? "text-emerald-200" : "text-slate-500"}`}>
                        ...
                        {slot.teamId ? (
                          <div className="flex flex-col min-w-0">
                            <div className="flex items-center gap-1.5 mb-0.5 min-w-0">
                              <span className={`text-[10px] md:text-[11px] font-black uppercase tracking-wider truncate ${slot.checkedIn ? "text-emerald-600" : isHighlighted ? "text-white" : "text-emerald-400"}`}>
                                {slot.teamId}
                              </span>
                              {slot.checkedIn && <ShieldCheck size={10} className="text-emerald-500 shrink-0" />}
                            </div>
                            <span className={`text-[10px] md:text-[11px] font-bold truncate leading-none ${slot.checkedIn ? "text-slate-900" : "text-white/90"}`}>
                              {slot.teamName}
                            </span>
                          </div>
                        ) : (
        ...
        {isScanning && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-sm bg-black rounded-[48px] p-1 shadow-2xl border border-white/10 overflow-hidden"
            >
              ...
              <div className="p-8 text-center bg-black border-t border-white/5">
                <p className="text-[10px] font-black text-white/40 uppercase tracking-[0.4em] leading-relaxed">
                  Center mission code within markers<br/>Verification is instantaneous
                </p>
              </div>
            </motion.div>
          </div>
        ...
        {scanError && (
          ...
          >
            ...
            <div>
              ...
              <p className="text-[10px] font-bold opacity-80">Camera access was denied or failed to initialize.</p>
            </div>
