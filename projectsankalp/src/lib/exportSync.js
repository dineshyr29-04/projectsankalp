...
function makePaymentSheet(teams) {
  const data = teams.map(team => ({
    ...
    'Payment Status': (team.paymentStatus || team.paymentStatus === 0) ? (team.paymentStatus === 'VERIFIED' || team.paymentVerified ? 'Verified' : 'Pending') : (team.paymentVerified ? 'Verified' : 'Pending'),
    'Transaction ID': team.transactionId || '-',
    'Verified By': team.verifiedBy || '-',
    'Verification Date': team.verificationDate ? new Date(team.verificationDate).toLocaleString() : '-',
    'Notes': team.notes || '-'
  }));
  ...
  ws['!cols'] = [ { wch: 12 }, { wch: 30 }, { wch: 18 }, { wch: 15 }, { wch: 20 }, { wch: 22 }, { wch: 30 } ];
  return wb;
}
...
function makeRegistrationSheet(teams) {
  ...
  ws['!cols'] = [ { wch: 6 }, { wch: 12 }, { wch: 30 }, { wch: 18 }, { wch: 16 }, { wch: 20 }, { wch: 14 }, { wch: 20 } ];
  return wb;
}
...
function makeDomainSummary(teams) {
  ...
  teams.forEach(team => {
    ...
    stats[d].total += 1;
    const isVerified = team.paymentStatus === 'VERIFIED' || team.paymentVerified;
  ...
  ws['!cols'] = [ { wch: 25 }, { wch: 12 }, { wch: 18 }, { wch: 12 }, { wch: 14 } ];
  return wb;
}
      ...
      timer = setTimeout(async () => {
        ...
        try {
          ...
          await Promise.all([
            ...
            uploadWorkbook(wbRegistration, 'exports/registration_checkin.xlsx'),
